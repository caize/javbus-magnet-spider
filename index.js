...
function maintask() {
    ...
    tools.get(config.urlpre + 'page/' + page, '', function (err, res) {
        if (err) {
            logger.log(err);
            //炸了，再执行自己
            if (err !== 404) {
        ...
        async.mapLimit(previewObj, threads, function (obj, callback) {
            ...
            let pre=config.urlpre.replace("http://", "").replace("https://", "");
...
function saveMagnet(obj, query) {
    let promise = new Promise(function (resolve, reject) {
        let {url, title, name}=obj;
        //获取磁力
        tools.get(config.indexUrl + "ajax/uncledatoolsbyajax.php?" + query, url, function (err, res) {
            if (err) {
                logger.log(err);
                reject({});
                return false;
